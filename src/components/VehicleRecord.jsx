...
/*
 * A vehicle's record in full — RC, eChallans and FASTag — as the Pravesha
 * panel lays it out. Used by "Check a vehicle" (a fresh lookup) and by the
 * Vehicles screen (the record already held), so the two always read the same.
 */
...
export const EXPIRY_KEY = /upto|expir|valid_?(till|to)/i;
const TONE = {
...
export const BAD_STATUS = /expired|inactive|blacklist|suspend|cancel|surrender|seiz|stolen|closed/i;
...
export const isTwoWheeler = (rc) => Boolean(rc) && TWO_WHEELER.test(`${rc.vehicle_class || ''} ${rc.vehicle_category || ''} ${rc.body_type || ''}`);
...
export const Section = ({ title, children, note, right, delay = 0 }) => (
...
export function ChallanView({ body }) {
...
export const FastagNotApplicable = () => (
...
export function FastagView({ body }) {
  const f = body.fastag || {};
